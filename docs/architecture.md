...
- **View-mode routing via URL hash:** GM vs. Player scene is determined by a hash route — `#/daggerheart/gm` vs. `#/daggerheart/player`. Hash routing (not a path route) keeps `base: './'` intact so the same build works on the Pages subpath, from `file://`, and in OBS with no server rewrites. No routing library required — `useViewMode` reads the hash (subscribed via `useSyncExternalStore`) and a single conditional at the root renders the scene. A URL that addresses **no** scene (the bare route, or an unrecognized segment) resolves to `null` and renders the **scene selector** — a game + role picker — rather than defaulting to the Player HUD, so every scene is reachable by navigation. Legacy `?mode=gm` query URLs still resolve for back-compat.
- **Setup → launch is the scene lifecycle, uniformly:** every scene renders a **setup** view until it is **launched**, then the **live** view, with an *edit* path back to setup. `SceneShell` (`src/scenes/SceneShell.tsx`) owns that lifecycle; a scene supplies only its two views plus a readiness rule, so adding a scene does not re-implement the flow. Two rules make it safe in practice:
  - **Launch is persisted per scene** (`vellum:launched:<scene>`), so a reload never re-gates a live screen. `null` (never recorded) is distinct from `false`, letting each scene pick a default — the Player scene treats an existing character as already-launched so upgrading users skip setup, while the GM scene defaults to showing its how-to first.
  - **The OBS browser source skips the launch gate.** That source is non-interactive by default, so gating the HUD behind a click there would burn a setup page into a live camera with no way to dismiss it. Only the *launch* gate is skipped, though: an explicit **edit** still opens setup inside OBS, since a click on the source is exactly the case where the operator wants it, and every setup view carries its own way back to live. Readiness is still enforced, so a scene that genuinely cannot render (a player with no character) stays on setup even inside OBS.

  A scene whose setup has nothing to configure still has one: the GM setup view is the **how-to** — what the screen shows, how to drive it during play, and how to get it into OBS.
...
- **Fixed design canvas, scaled to fit the source (REQ-027):** every scene is laid out in 1920×1080 canvas pixels — the HUD's corner anchors, the type scale, the pip sizes — and `Stage` (`src/components/Stage/`) scales that whole canvas by `min(vw/1920, vh/1080)` to fit the viewport it lands in, centring it and letterboxing the remainder transparently. The canvas is never made fluid: keeping it fixed is what lets components be authored against one coordinate system and stay legible at Discord tile scale. This matters because an OBS browser source renders at whatever width/height the user typed into its properties — OBS's own default is 800×600 — and any source narrower than 1920 previously showed the *left slice* of a 1920-wide layout with the right edge of the UI clipped off. Scaling up is deliberate too, so a 2560×1440 source fills its frame instead of rendering a 1920 island. Two consequences for component CSS: the stage is a transformed ancestor, so `position: fixed` descendants are contained by the *canvas* rather than the viewport, and **`vw`/`vh` units must not be used inside it** — they measure the un-scaled browser source, not the canvas; use percentages of the stage instead.
...
| **View mode** | The URL hash route (`#/daggerheart/gm` / `#/daggerheart/player`) that determines whether the HUD renders the Player stat layout or the GM layout. A URL addressing no scene renders the scene selector instead. Legacy `?mode=gm` query URLs still resolve. |
...
**Notes:** the `#/daggerheart/gm` hash route (legacy `?mode=gm` still resolves) activates GM rendering mode. `useViewMode` reads the scene from the URL hash; no player-stat elements mounted. Fear max = 12 from Daggerheart system config. Fear is **GM state, not character state**: it persists under its own `vellum:fear` key via `useFear`, so the GM scene renders with no character configured. StatTrack Fear instance.
